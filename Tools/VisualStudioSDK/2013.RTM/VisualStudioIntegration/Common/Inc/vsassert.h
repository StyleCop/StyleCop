...
/*
 ...
 Assert ETW provider 
 Built only in retail if VS_RETAIL_ASSERTS is defined.

 Provider Guid: {E5328C6F-4C94-45F7-ACAF-640C6A447654}
 Events:
 ...
 Perf impact:
   To capture the perf impact of vs asserts in retail, define VS_RETAIL_ASSERTS_PERF_COST.
   When enabled, each individual VSASSERT will be collecting its usage cost in QPC ticks.

   If the individual cost or per-module cost exceeds VS_RETAIL_ASSERTS_PERF_TRESHOLD_MS (ms), 
   additional assert ETW event will fire with Assert_Condition equals
   "Evaluation cost of all asserts in module" or "Evaluation cost of assert"
   and Assert_Message equals the string with the total number of ms.
   This total number of ms accumulates over the time of the IDE session.
...
#define DF_NOARGS                0x40 // Indicates that the message string does not take arguments, even if the string contains "%" characters.
...
CLINKAGE BOOL ENTRYPOINT VsEnableAsserts (BOOL fEnable);
...
// "CAssert".  This is a compile time assert that will fire if fTest is zero.
#define VSCASSERT(fTest, msg)  { static_assert(fTest, #msg); }
...
//=--------------------------------------------------------------------------=
// Retail asserts as ETW events
//
// With VS_RETAIL_ASSERTS defined VSASSERT in retail will check if there is 
// VS Assert ETW subscriber (for provider Guid: {E5328C6F-4C94-45F7-ACAF-640C6A447654}), 
// and if there is, VSASSERT will evaluate the assert conditions and fire ETW events
// If there is no subscriber, assert conditions are not evaluated.
//
#ifdef VS_RETAIL_ASSERTS

#include <tchar.h>
#include <sal.h>

#ifdef VS_RETAIL_ASSERTS_PERF_COST

#include <stdlib.h>

// Assert perf threshold (ms). 
// This is the number of milliseconds spent in evaluating assert conditions, per assert or per module
// that, when exceeded, will trigger "Evaluation cost of assert" assert ETW events to fire
#ifndef VS_RETAIL_ASSERTS_PERF_TRESHOLD_MS
#define VS_RETAIL_ASSERTS_PERF_TRESHOLD_MS 20
#endif

#endif // VS_RETAIL_ASSERTS_PERF_COST

class AssertEtwProvider
{
private:
	typedef bool (__stdcall *PFNAreAssertsEnabled) ();
	typedef void (__stdcall *PFNWriteAssertEtwEventA) (LPCSTR pszAssert, LPCSTR pszMsg, LPCSTR pszFile, UINT line);

    PFNAreAssertsEnabled pfnAreAssertsEnabled;
	PFNWriteAssertEtwEventA pfnWriteAssertEtwEventA;

#if defined(__cplusplus) && !defined(VSASSERT_MIXED_LINKAGE)
	typedef void (__stdcall *PFNWriteAssertEtwEventW) (LPCSTR pszAssert, LPCWSTR pwszMsg, LPCSTR pszFile, UINT line);

private:
	PFNWriteAssertEtwEventW pfnWriteAssertEtwEventW;
#endif

#ifdef VS_RETAIL_ASSERTS_PERF_COST

private:
    // Individual VSAssert cost threshold in ms after which that assert will signal that it's to expensive.
    LONGLONG nCostThreshold;
    LONGLONG nQpcFrequency;

#endif

    // Ctor
	AssertEtwProvider(void)
	{
        // Get the assert routines from the appid module
		this->pfnAreAssertsEnabled = nullptr;
		this->pfnWriteAssertEtwEventA = nullptr;

#if defined(__cplusplus) && !defined(VSASSERT_MIXED_LINKAGE)
		this->pfnWriteAssertEtwEventW = nullptr;
#endif

		HMODULE hModule = ::GetModuleHandle(nullptr);
		if (hModule != nullptr)
		{
			this->pfnAreAssertsEnabled = reinterpret_cast<PFNAreAssertsEnabled>(::GetProcAddress(hModule, sizeof(void *) == 4 ? "_IsAssertEtwEnabled@0" : "IsAssertEtwEnabled"));
			if (this->pfnAreAssertsEnabled != nullptr)
			{
				this->pfnWriteAssertEtwEventA = reinterpret_cast<PFNWriteAssertEtwEventA>(::GetProcAddress(hModule, sizeof(void *) == 4 ? "_WriteAssertEtwEventA@16" : "WriteAssertEtwEventA"));

#if defined(__cplusplus) && !defined(VSASSERT_MIXED_LINKAGE)
				this->pfnWriteAssertEtwEventW = reinterpret_cast<PFNWriteAssertEtwEventW>(::GetProcAddress(hModule, sizeof(void *) == 4 ? "_WriteAssertEtwEventW@16" : "WriteAssertEtwEventW"));
#endif
			}
		}

        // Get the QPC frequency and cost threshold
#ifdef VS_RETAIL_ASSERTS_PERF_COST

        LARGE_INTEGER nFrequency;
        if (::QueryPerformanceFrequency(&nFrequency))
        {
            this->nQpcFrequency = nFrequency.QuadPart;
            this->nCostThreshold = (this->nQpcFrequency * (VS_RETAIL_ASSERTS_PERF_TRESHOLD_MS)) / 1000;
        }
        else
        {
            this->nQpcFrequency = 0;
        }

#endif // VS_RETAIL_ASSERTS_PERF_COST
	}

    // Gets singleton instance - this method is not thread safe
    static AssertEtwProvider &InternalGetInstance()
    {
        static AssertEtwProvider instance;
        return instance;
    }
...
public:
	...
	static AssertEtwProvider& GetInstance()
	{
        volatile static long initialized = 0;

        // Chev if the singelton has not initialized yet.
        if (initialized != 2)
        {
            // Check first if another thread is not initializing it now.
            if (InterlockedCompareExchange(&initialized, 1, 0) == 0)
            {
                // We're the first thread to start initializing - do it now.
                AssertEtwProvider &result = InternalGetInstance();
	...
	// ----------------------------------------------------------------------------
	// Gets the value indicating whether the Assert ETW provider is enabled.
	// ----------------------------------------------------------------------------
	bool AreAssertsEnabled()
	{
		return this->pfnAreAssertsEnabled != nullptr && this->pfnAreAssertsEnabled();
	}

	// ----------------------------------------------------------------------------
	// Write an Assert Event to the Assert ETW provider
	// ----------------------------------------------------------------------------
#if defined(__cplusplus) && !defined(VSASSERT_MIXED_LINKAGE)

	void WriteEtwEvent(
        _In_opt_z_ LPCSTR pszAssert,
        _In_opt_z_ LPCWSTR pszMsg,
        _In_opt_z_ LPCSTR pszFile,
        _In_ UINT line)
	{
		if (this->pfnWriteAssertEtwEventW != nullptr)
		{
			this->pfnWriteAssertEtwEventW(pszAssert, pszMsg, pszFile, line);
		}
	}
	...
	void WriteEtwEvent(
        _In_opt_z_ LPCSTR pszAssert,
        _In_opt_z_ LPCSTR pszMsg,
        _In_opt_z_ LPCSTR pszFile,
        _In_ UINT line)
	{
		if (this->pfnWriteAssertEtwEventA != nullptr)
		{
			this->pfnWriteAssertEtwEventA(pszAssert, pszMsg, pszFile, line);
		}
	}
...
private:
    ...
    static LONGLONG InterlockedAdd64(
        _Inout_ volatile LONGLONG *pnTarget, 
        _In_ LONGLONG nIncrement)
    {
        while (true)
        {
            LONGLONG nOldValue = ::InterlockedCompareExchange64(pnTarget, 0, 0);
            LONGLONG nNewValue = nOldValue + nIncrement;
            if (::InterlockedCompareExchange64(pnTarget, nNewValue, nOldValue) == nOldValue)
            {
                return nNewValue;
            }
        } 
    ...
    void IncrementCostAndFireCostEvent(
        _Inout_ volatile LONGLONG *pnTotalCost,
        _Inout_ volatile LONGLONG *pnTotalCostWhenCostEventFired,
        _In_ LONGLONG nCost,
        _In_opt_z_ LPCSTR pszCostAssert,
        _In_opt_z_ LPCSTR pszFile, 
        _In_ UINT nLine)

    {
        if (this->nQpcFrequency == 0 || pnTotalCost == nullptr || pnTotalCostWhenCostEventFired == nullptr || nCost == 0)
        {
            return;
        }

        LONGLONG nTotalCost = InterlockedAdd64(pnTotalCost, nCost);
        LONGLONG nTotalCostWhenCostEventFired = InterlockedAdd64(pnTotalCostWhenCostEventFired, nCost);

        static volatile LONG nCostEventIsFiring = 0;

        // Fire infrastructure cost event
        if (nTotalCostWhenCostEventFired >= this->nCostThreshold && ::InterlockedCompareExchange(&nCostEventIsFiring, 1, 0) == 0)
        {
            char szTotalCostBuffer[32];
            LONGLONG nTotalCostMs = (nTotalCost * 1000) / this->nQpcFrequency;
            if (::_i64toa_s(nTotalCostMs, szTotalCostBuffer, _countof(szTotalCostBuffer), 10 /* radix */) == 0)
            {
                this->WriteEtwEvent(pszCostAssert, szTotalCostBuffer, pszFile, nLine);
            }

            ::InterlockedExchange64(pnTotalCostWhenCostEventFired, 0);
            ::InterlockedExchange(&nCostEventIsFiring, 0);
        }
    }
...
public:
	...
	// ----------------------------------------------------------------------------
	// Adds assert QPC cost and fires cost events if appropriate
	// ----------------------------------------------------------------------------
    void AddAssertQpcCost(
        _In_ LARGE_INTEGER &qpcStart, 
        _Inout_ LONGLONG *pnTotalCost, 
        _Inout_ LONGLONG *pnTotalCostWhenCostEventFired,
        _In_opt_z_ LPCSTR pszFile, 
        _In_ UINT nLine)
    {
        LARGE_INTEGER qpcFinish;                                    
        ::QueryPerformanceCounter(&qpcFinish);
        LONGLONG nCost = qpcFinish.QuadPart - qpcStart.QuadPart;

        static volatile LONGLONG nAllAssertsTotalCost = 0;
        static volatile LONGLONG nAllAssertsTotalCostWhenCostEventFired = 0;

        this->IncrementCostAndFireCostEvent(
            &nAllAssertsTotalCost,
            &nAllAssertsTotalCostWhenCostEventFired,
            nCost,
            "Evaluation cost of all asserts in module",
            nullptr, /* pszFile */
            0        /* nLine */);

        this->IncrementCostAndFireCostEvent(
            pnTotalCost,
            pnTotalCostWhenCostEventFired,
            nCost,
            "Evaluation cost of assert",
            pszFile,
            nLine);
    }
...
// VSASSERT as ETW performance costing if required
#ifdef VS_RETAIL_ASSERTS_PERF_COST
#define VS_RETAIL_ASSERTS_PERF_COST_START LARGE_INTEGER qpcStart;                   \
    ::QueryPerformanceCounter(&qpcStart);                                      \

#define VS_RETAIL_ASSERTS_PERF_COST_FINISH                                          \
    static LONGLONG nTotalCost = 0;                                            \
    static LONGLONG nTotalCostWhenCostEventFired = 0;                          \
    etwProvider.AddAssertQpcCost(                                              \
       qpcStart,                                                               \
       &nTotalCost,                                                            \
       &nTotalCostWhenCostEventFired,                                          \
       __FILE__,                                                               \
       __LINE__);                                                              \

#else
#define VS_RETAIL_ASSERTS_PERF_COST_START    do {} while (0)
#define VS_RETAIL_ASSERTS_PERF_COST_FINISH   do {} while (0)
#endif


// Note: assert condition fTest is only evaluated in retail if the assert ETW provider is enabled.
#define VSASSERT(fTest, szMsg)                                                                  \
    do                                                                                          \
    {                                                                                           \
        AssertEtwProvider &etwProvider = AssertEtwProvider::GetInstance();                      \
        if (etwProvider.AreAssertsEnabled())                                                    \
        {                                                                                       \
            VS_RETAIL_ASSERTS_PERF_COST_START;                                                  \
            if (!(fTest))                                                                       \
            {                                                                                   \
                etwProvider.WriteEtwEvent(#fTest, szMsg, __FILE__, __LINE__);                   \
            }                                                                                   \
            VS_RETAIL_ASSERTS_PERF_COST_FINISH;                                                 \
        }                                                                                       \
    }                                                                                           \
    while (false)                                                                               \

// Note: verify condition fTest is always evaluated, regardless whether the assert ETW provider is enabled.
#define VSVERIFY(fTest, szMsg) VSVERIFYFUNC(fTest, szMsg, _DebugNop)
#define VSFAIL(szMsg) VSASSERT(0, szMsg)
#define VSIMPLIES(fHypothesis, fConclusion, szMsg) VSASSERT(!(fHypothesis) || (fConclusion), szMsg)
