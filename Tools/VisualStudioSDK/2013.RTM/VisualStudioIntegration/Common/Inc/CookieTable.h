 ...
 *
 *--------------------------------------------------------------------------*/
#ifndef _COOKIETABLE_H_
#define _COOKIETABLE_H_
...
public:
    CCookieTable(bool fLockDuringForEach = true)
        : m_cLocks (0)
        , m_fLockDuringForEach (fLockDuringForEach)
    {
        static_assert (CookieTraits::MinCookie < CookieTraits::MaxCookie,
                       "The min and max cookies in CookieTraits are out of range");

        static_assert ((CookieTraits::InvalidCookie < CookieTraits::MinCookie) ||
                       (CookieTraits::InvalidCookie > CookieTraits::MaxCookie),
                       "CookieTraits defines the invalid cookie between min and max cookies");
    }
    ...
    Func for_each (Func func, bool fIncludeInvalidElements = false)
    {
        ...
        if (m_fLockDuringForEach)
        {
            ...
            for (auto& pair : *this)
            {
                const cookie_type& cookie = pair.first;
        ...
        else
        {
            ...
            {
                /*
                 * instead of locking the table, take a snapshot of it and iterate over the snapshot
                 */
                std::vector<std::pair<cookie_type, mapped_type>> snapshot(begin(), end());
                ...
                for (auto& pair : snapshot)
                {
                    ...
                    /*
                     * since we've taken a snapshot, there's no way for an element to be 
                     * invalidated, and there's no reason to check validity before calling func
                     */
                    func (cookie, value);
                }
            }
...
private:
    ...
    const bool                          m_fLockDuringForEach;
