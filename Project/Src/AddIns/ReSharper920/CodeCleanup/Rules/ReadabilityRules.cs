...
namespace StyleCop.ReSharper920.CodeCleanup.Rules
{
    ...
    using StyleCop.ReSharper920.Extensions;
    ...
    internal class ReadabilityRules
    {
        ...
        private static void ProcessLocalVariableDeclaration(ILocalVariableDeclaration localVariableDeclaration)
        {
            ...
            if (multipleDeclaration.Declarators.Count > 1)
            {
                ...
                using (WriteLockCookie.Create(true))
                {
                    multipleDeclaration.SetTypeUsage(CSharpElementFactory.GetInstance(localVariableDeclaration.GetPsiModule()).CreateTypeUsageNode(newType, localVariableDeclaration));
                }
            }
            else
            {
