...
namespace StyleCop.ReSharper920.CodeCleanup.Rules
{
    ...
    using StyleCop.ReSharper920.Options;
    ...
    public class DocumentationRules
    {
        ...
        private void UpdateFileHeader(DocumentationOptions options, ICSharpFile file)
        {
            ...
            string fileName = file.GetSourceFile().ToProjectFile().Location.Name;
            UpdateFileHeaderStyle updateFileHeaderOption;

            if (!Enum.TryParse(options.SA1633SA1641UpdateFileHeader, true, out updateFileHeaderOption) ||
                updateFileHeaderOption == UpdateFileHeaderStyle.Ignore)
            {
                return;
            }
