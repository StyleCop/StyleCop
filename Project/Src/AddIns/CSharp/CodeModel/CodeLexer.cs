...
namespace Microsoft.StyleCop.CSharp.CodeModel
{
    ...
    internal partial class CodeLexer
    {
        ...
        internal List<Symbol> GetSymbols(CsDocument document, IDictionary<string, object> preprocessorDefinitions)
        {
            ...
            while (true)
            {
                Symbol symbol = this.GetSymbol(document, preprocessorDefinitions, true);
                if (symbol == null)
                {
                    break;
                }
        ...
        internal Symbol GetSymbol(CsDocument document, IDictionary<string, object> preprocessorDefinitions, bool evaluatePreprocessors)
        {
            Param.Ignore(preprocessorDefinitions);
            Param.AssertNotNull(document, "document");
            ...
            if (firstCharacter != char.MinValue)
            {
                switch (firstCharacter)
                {
                    ...
                    case '#':
                        symbol = this.GetPreprocessorDirectiveSymbol(document, preprocessorDefinitions, evaluatePreprocessors);
                        break;
        ...
        private Symbol GetPreprocessorDirectiveSymbol(CsDocument document, IDictionary<string, object> preprocessorDefinition, bool evaluate)
        {
            Param.AssertNotNull(document, "document");
            ...
            // Create the symbol.
            var symbol = new Symbol(text.ToString(), SymbolType.PreprocessorDirective, location);
            ...
            if (evaluate && preprocessorDefinition != null)
            {
                ...
                this.CheckForConditionalCompilationDirective(document, symbol, preprocessorDefinition);
            }
        ...
        /// <param name="preprocessorDefinitions">The active configuration.</param>
        private void CheckForConditionalCompilationDirective(CsDocument document, Symbol preprocessorSymbol, IDictionary<string, object> preprocessorDefinitions)
        {
            Param.AssertNotNull(document, "document");
            ...
            else
            {
                    ...
                    document, preprocessorSymbol, preprocessorDefinitions, bodyIndex, type, out skip))
                {
                    ...
                    if (skip)
                    {
                        ...
                        {
                            // Get the next symbol.
                            Symbol symbol = this.GetSymbol(document, preprocessorDefinitions, false);
                            if (symbol == null)
                            {
                                throw new SyntaxException(document, preprocessorSymbol.LineNumber);
                            }
