...
namespace StyleCop.CSharp.CodeModel
{
    ...
    internal partial class CodeParser
    {
        ...
        /// <returns>Returns the token.</returns>
        private OperatorSymbolToken GetOperatorSymbolToken(CodeUnitProxy parentProxy, OperatorType? operatorType = null)
        {
            Param.AssertNotNull(parentProxy, "parentProxy");
            Param.Ignore(operatorType);

            this.AdvanceToNextCodeSymbol(parentProxy);
            OperatorSymbolToken operatorToken = this.PeekOperatorSymbolToken(operatorType);

            // Advance past the operator symbol.
            this.symbols.Advance();
            parentProxy.Children.Add(operatorToken);

            if (operatorToken == null || (operatorType != null && operatorToken.SymbolType != operatorType))
            {
                throw this.CreateSyntaxException();
            }
        ...
        /// </summary>
        /// <param name="hint">Optional hint indicating the type of operator token we think we will find.</param>
        /// <returns>Returns the token.</returns>
        private OperatorSymbolToken PeekOperatorSymbolToken(OperatorType? hint = null)
        {
            Param.Ignore(hint);
            ...
            OperatorSymbolToken operatorToken = this.CreateOperatorSymbolToken(operatorSymbol, hint, this.symbols.Generated);
        ...
        /// </summary>
        /// <param name="symbol">The symbol to convert.</param>
        /// <param name="hint">Optional hint indicating what type of operator symbol we think should be created.</param>
            ...
            if (index != -1)
            {
                ...
                if (symbol.SymbolType == SymbolType.OpenSquareBracket)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            typeTokenProxy.Children.Add(new SkippedSection(this.document, symbol.Location, this.symbols.Generated, symbol.Text));
                            ++startIndex;
                        }
                        ...
                        else if (symbol.SymbolType == SymbolType.Other)
                        {
                            ...
                            typeTokenProxy.Children.Add(this.ConvertTokenSymbol(symbol, TokenType.Literal));
                            ++startIndex;
                        }
