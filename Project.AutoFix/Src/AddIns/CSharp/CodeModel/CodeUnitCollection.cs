...
namespace StyleCop.CSharp.CodeModel
{
    ...
    public class CodeUnitCollection : ICollection<CodeUnit>
    {
        ...
        /// </summary>
        /// <param name="item">The item to add to the collection.</param>
        /// <param name="itemToInsertAfter">The item to insert the new item after.</param>
        internal void InsertAfter(CodeUnit item, CodeUnit itemToInsertAfter)
        {
            Param.AssertNotNull(item, "item");
            Param.Ignore(itemToInsertAfter, "itemToInsertAfter");

            CsLanguageService.Debug.Assert(itemToInsertAfter.Parent != null, "Item is not in this collection.");
            CsLanguageService.Debug.Assert(itemToInsertAfter.Parent.Children == this, "Item is not in this collection.");
            ...
            this.items.InsertAfter(item, itemToInsertAfter);
            item.ParentReference = this.parentReference;
        }
        ...
        /// </summary>
        /// <param name="item">The item to add to the collection.</param>
        internal void InsertLast(CodeUnit item)
        {
            Param.AssertNotNull(item, "item");
            this.Add(item, true);
        }
        ...
        internal bool Remove(CodeUnit item)
        {
            ...
            {
                CsLanguageService.Debug.Fail("The code unit is not a member of the collection.");
                return false;
            }
        ...
        private class CodeUnitLinkedList : LinkedItemList<CodeUnit>
        {
            ...
            public override void InsertBefore(CodeUnit node, CodeUnit nodeToInsertBefore)
            {
                ...
                base.InsertBefore(node, nodeToInsertBefore);
                this.IncrementOrDecrementCount(node, 1);
            }
