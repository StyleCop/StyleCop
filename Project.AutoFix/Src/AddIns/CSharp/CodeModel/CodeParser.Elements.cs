//-----------------------------------------------------------------------
// <copyright file="CodeParser.Elements.cs">
//     MS-PL
// </copyright>
// <license>
    ...
    internal partial class CodeParser
    {
        ...
        private static bool SanityCheckElementTypeAgainstParent(ElementType elementType, Element parent)
        {
            ...
            switch (elementType)
            {
                ...
                default:
                    CsLanguageService.Debug.Fail("Unexpected element type.");
                    break;
            }
        ...
        private void AddElementToPartialElementsList(Element element)
        {
            Param.AssertNotNull(element, "element");

            IPartialElementsService partialElements = this.languageService.PartialElementService;
            if (partialElements != null)
            {
                partialElements.TryAdd(element);
            }
        }
        ...
        private void ParseElementContainer(CodeUnitProxy elementProxy, Element element, bool unsafeCode)
        {
            ...
            BracketToken closingBracket = this.ParseElementContainerBody(elementProxy, element, unsafeCode);
        ...
        private BracketToken ParseElementContainerBody(CodeUnitProxy elementProxy, Element element, bool unsafeCode)
        {
            Param.AssertNotNull(elementProxy, "elementProxy");
            Param.AssertNotNull(element, "element");
            Param.Ignore(unsafeCode);
            ...
            while (true)
            {
                ...
                // Parse the element.
                Element childElement = this.GetElement(childElementProxy, childElementType.Value, element, unsafeCode, generated, attributes);
                elementProxy.Children.Add(childElement);
                     
                // Add the element to the collection of partial elements, if necessary.
                this.AddElementToPartialElementsList(childElement);
            }
        ...
        private ElementHeader GetXmlHeader()
        {
            // Get the first symbol and make sure it is the right type.
            int index = 1;
            Symbol firstSymbol = this.symbols.Peek(index);
            CsLanguageService.Debug.Assert(firstSymbol != null && firstSymbol.SymbolType == SymbolType.XmlHeaderLine, "Expected an xml documentation header line");
            ...
            CsLanguageService.Debug.Assert(end != -1, "Should have advanced at least one symbol");
            ...
            for (int i = 1; i <= end; ++i)
            {
                ...
                {
                    CsLanguageService.Debug.Fail("Each symbol should be added successfully since the types were verified above.");
                }
            }

            // Create the Xml header object.
            return new ElementHeader(xmlHeaderProxy);
        }
            ...
            ICollection<Attribute> attributes)
        {
            ...
            {
                case ElementType.Namespace:
                    return this.GetNamespace(elementProxy, unsafeCode, generated, attributes);
                ...
                case ElementType.Struct:
                case ElementType.Interface:
                    return this.GetClass(elementProxy, elementType, parentElement, unsafeCode, generated, attributes);
        ...
        /// <param name="attributes">The attributes on the element.</param>
        /// <returns>Returns the element.</returns>
        private Namespace GetNamespace(CodeUnitProxy elementProxy, bool unsafeCode, bool generated, ICollection<Attribute> attributes)
        {
            Param.AssertNotNull(elementProxy, "elementProxy");
            Param.Ignore(unsafeCode);
            Param.Ignore(generated);
            ...
            this.ParseElementContainer(elementProxy, @namespace, unsafeCode);
            ...
            ICollection<Attribute> attributes)
        {
            ...
            {
                CsLanguageService.Debug.Assert(keywordType == TokenType.Interface, "Invalid element type");
                item = new Interface(elementProxy, name.Text, attributes, typeConstraints, unsafeCode);
            }

            // Parse the body of the element.
            this.ParseElementContainer(elementProxy, item, unsafeCode);
        ...
        private Property GetProperty(CodeUnitProxy elementProxy, Element parent, bool unsafeCode, bool generated, ICollection<Attribute> attributes)
        {
            ...
            this.ParseElementContainer(elementProxy, property, unsafeCode);
        ...
        private Indexer GetIndexer(CodeUnitProxy elementProxy, Element parent, bool unsafeCode, bool generated, ICollection<Attribute> attributes)
        {
            ...
            this.ParseElementContainer(elementProxy, indexer, unsafeCode);
        ...
        private Accessor GetAccessor(CodeUnitProxy elementProxy, Element parent, bool unsafeCode, bool generated)
        {
            ...
            CsLanguageService.Debug.Assert(accessor != null, "Accessor not created.");
        ...
        private ICollection<TypeParameterConstraintClause> GetTypeConstraintClauses(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            while (symbol.Text == "where")
            {
                ...
                while (true)
                {
                    ...
                    {
                        this.AdvanceToNextCodeSymbol(constraintClauseProxy);
        ...
        private Token GetElementNameToken(CodeUnitProxy parentProxy, bool unsafeCode, bool allowArrayBrackets)
        {
            ...
            else
            {
                ...
                CsLanguageService.Debug.Assert(typeToken != null && typeToken.Children.Count > 0, "The name token is invalid.");
                ...
                {
                    Token nameToken = typeToken.FindFirstChildToken();
                    CsLanguageService.Debug.Assert(nameToken != null, "Expected a child token");
