    ...
    internal partial class CodeParser
    {
            ...
            if (symbol != null)
            {
                ...
                switch (symbol.SymbolType)
                {
                    ...
                    case SymbolType.OpenParenthesis:
                        ...
                        {
                            expression = this.GetOpenParenthesisExpression(expressionExtensionProxy, previousPrecedence, unsafeCode);
                        }
            ...
            CodeUnitProxy expressionExtensionProxy, Expression leftSide, ExpressionPrecedence previousPrecedence, bool unsafeCode, bool typeExpression, bool allowVariableDeclaration)
        {
            ...
            if (typeExpression)
            {
                ...
                {
                    expression = this.GetChildAccessExpression(expressionExtensionProxy, leftSide, previousPrecedence, unsafeCode);
                }
            }
            else
            {
                ...
                switch (symbol.SymbolType)
                {
                    ...
                    case SymbolType.Other:
                        ...
                        if (allowVariableDeclaration && 
                            (leftSide.ExpressionType == ExpressionType.Literal || 
                             leftSide.ExpressionType == ExpressionType.MemberAccess ||
                             leftSide.ExpressionType == ExpressionType.QualifiedAlias))
                        {
        ...
        private Expression GetOtherExpression(CodeUnitProxy parentProxy, bool allowVariableDeclaration, bool unsafeCode)
        {
            ...
            {
                CodeUnitProxy variableDeclarationExpressionProxy = new CodeUnitProxy(this.document);
                ...
                expression = this.GetVariableDeclarationExpression(variableDeclarationExpressionProxy, parentProxy, type, ExpressionPrecedence.None, unsafeCode);
            }
            else
            {
        ...
        /// <summary>
        /// Reads a child access expression.
        /// </summary>
        /// <param name="expressionProxy">Proxy object for the expression being created.</param>
        ...
        /// <param name="unsafeCode">Indicates whether the code is marked as unsafe.</param>
        /// <returns>Returns the expression.</returns>
        private ChildAccessExpression GetChildAccessExpression(
            CodeUnitProxy expressionProxy, Expression leftSide, ExpressionPrecedence previousPrecedence, bool unsafeCode)
        {
            ...
            ChildAccessExpression expression = null;
            Func<LiteralExpression, ChildAccessExpression> expressionCreator = null;
            ...
            if (symbol.SymbolType == SymbolType.Dot)
            {
                ...
                expressionCreator = (LiteralExpression rightSide) => { return new MemberAccessExpression(expressionProxy, leftSide, rightSide); };
            }
            else if (symbol.SymbolType == SymbolType.Pointer)
            {
                operatorType = OperatorType.Pointer;
                precedence = ExpressionPrecedence.Primary;
                expressionCreator = (LiteralExpression rightSide) => { return new PointerAccessExpression(expressionProxy, leftSide, rightSide); };
            }
            else if (symbol.SymbolType == SymbolType.QualifiedAlias)
            {
                ...
                expressionCreator = (LiteralExpression rightSide) => { return new QualifiedAliasExpression(expressionProxy, leftSide, rightSide); };
            }
            else
            {
            ...
            if (CheckPrecedence(previousPrecedence, precedence))
            {
                ...
                // Create the appropriate type of expression.
                expression = expressionCreator(member);
            }
            ...
            CodeUnitProxy parentProxy, Expression type, ExpressionPrecedence previousPrecedence, bool unsafeCode)
        {
            ...
            return this.GetVariableDeclarationExpression(new CodeUnitProxy(this.document), parentProxy, type, previousPrecedence, unsafeCode);
        }
        ...
        /// <param name="unsafeCode">Indicates whether the code being parsed resides in an unsafe code block.</param>
        /// <returns>Returns the expression.</returns>
        private Expression GetOpenParenthesisExpression(CodeUnitProxy parentProxy, ExpressionPrecedence previousPrecedence, bool unsafeCode)
        {
            Param.AssertNotNull(parentProxy, "parentProxy");
        ...
        private ArgumentList GetArgumentList(CodeUnitProxy expressionProxy, SymbolType closingSymbol, bool unsafeCode)
        {
            ...
            var argumentListProxy = new CodeUnitProxy(this.document);
            ...
            while (symbol != null)
            {
                ...
                var argumentProxy = new CodeUnitProxy(this.document);
            ...
            CodeUnitProxy expressionProxy, Expression leftHandSide, ExpressionPrecedence previousPrecedence, bool unsafeCode)
        {
            ...
            if (CheckPrecedence(previousPrecedence, ExpressionPrecedence.Relational))
            {
                ...
                CsLanguageService.Debug.Assert(leftHandSide.Children.Count > 0, "The left hand side should not be empty");
            ...
            CodeUnitProxy expressionProxy, Expression leftHandSide, ExpressionPrecedence previousPrecedence)
        {
            ...
            if (CheckPrecedence(previousPrecedence, ExpressionPrecedence.Primary))
            {
                ...
                // Create and return the expression.
                expression = new PostfixIncrementExpression(expressionProxy, leftHandSide);
            }
            ...
            CodeUnitProxy expressionProxy, Expression leftHandSide, ExpressionPrecedence previousPrecedence)
        {
            ...
            if (CheckPrecedence(previousPrecedence, ExpressionPrecedence.Primary))
            {
                ...
                // Create and return the expression.
                expression = new PostfixDecrementExpression(expressionProxy, leftHandSide);
            }
        ...
        private IncrementExpression GetUnaryIncrementExpression(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            // Create and return the expression.
            var expression = new PrefixIncrementExpression(expressionProxy, valueExpression);
            parentProxy.Children.Add(expression);
        ...
        private DecrementExpression GetUnaryDecrementExpression(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            CodeUnitProxy expressionProxy = new CodeUnitProxy(this.document);
            ...
            // Create and return the expression.
            var expression = new PrefixDecrementExpression(expressionProxy, valueExpression);
            parentProxy.Children.Add(expression);
            ...
            CodeUnitProxy expressionProxy, Expression leftHandSide, ExpressionPrecedence previousPrecedence, bool unsafeCode)
        {
            ...
            // Read the details of the expression.
            OperatorSymbolToken operatorToken = this.PeekOperatorSymbolToken();
            CsLanguageService.Debug.Assert(operatorToken.SymbolType == OperatorType.NullCoalescingSymbol, "Expected a null-coalescing symbol");
        ...
        private Expression GetOperatorRightHandExpression(CodeUnitProxy parentProxy, ExpressionPrecedence precedence, bool unsafeCode)
        {
            ...
            CsLanguageService.Debug.Assert(rightHandSide.Children.Count > 0, "The right-hand side should not be empty.");
        ...
        private CollectionInitializerExpression GetAnonymousTypeInitializerExpression(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            for (Expression initializerExpression = initializer.FindFirstChildExpression(); 
                initializerExpression != null;
                initializerExpression = initializerExpression.FindNextSiblingExpression())
            {
                if (initializerExpression.ExpressionType != ExpressionType.Literal &&
                    ...
                    initializerExpression.ExpressionType != ExpressionType.QualifiedAlias &&
        ...
        private bool IsLambdaExpression()
        {
            ...
            {
                CsLanguageService.Debug.Fail("IsLambdaExpression called incorrectly.");
            }
        ...
        private QueryContinuationClause GetQueryContinuationClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            #if DEBUG
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "into", "Expected an into keyword");
            #endif
        ...
        private QueryFromClause GetQueryFromClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            #if DEBUG
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "from", "Expected a from keyword");
            #endif
        ...
        private QueryLetClause GetQueryLetClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            #if DEBUG
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "let", "Expected a let keyword");
            #endif
        ...
        private QueryWhereClause GetQueryWhereClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            #if DEBUG
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "where", "Expected a where keyword");
            #endif
        ...
        private QueryJoinClause GetQueryJoinClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            #if DEBUG
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "join", "Expected a join keyword");
            #endif
        ...
        private QuerySelectClause GetQuerySelectClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            #if DEBUG
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "select", "Expected a select keyword");
            #endif
        ...
        private QueryGroupClause GetQueryGroupClause(CodeUnitProxy parentProxy, bool unsafeCode)
        {
            ...
            Symbol symbol = this.PeekNextSymbol();
            CsLanguageService.Debug.Assert(symbol.SymbolType == SymbolType.Other && symbol.Text == "group", "Expected a group keyword.");
        ...
        private UnsafeAccessExpression GetUnsafeTypeExpression(CodeUnitProxy expressionProxy, Expression type, ExpressionPrecedence previousPrecedence)
        {
            ...
            if (CheckPrecedence(previousPrecedence, ExpressionPrecedence.Unary))
            {
                ...
                {
                    this.GetOperatorSymbolToken(expressionProxy, OperatorType.AddressOf);
                    return new AddressOfExpression(expressionProxy, type);
                }
                else if (symbol.SymbolType == SymbolType.Multiplication)
                {
                    this.GetOperatorSymbolToken(expressionProxy, OperatorType.Dereference);
                    return new DereferenceExpression(expressionProxy, type);
                }
                else
                {
                    CsLanguageService.Debug.Fail("Unexpected operator type.");
        ...
        private bool IsDereferenceExpression(Expression leftSide)
        {
            ...
            int index = this.GetNextCodeSymbolIndex(1);
            CsLanguageService.Debug.Assert(index > 0, "The next symbol must be a dereference symbol or a multiplication symbol.");

            Symbol symbol = this.symbols.Peek(index);
            CsLanguageService.Debug.Assert(symbol != null && symbol.SymbolType == SymbolType.Multiplication, "The next symbol must be a dereference symbol or a multiplication symbol.");
            ...
            if (leftSide != null)
            {
                if (leftSide is UnsafeAccessExpression)
                {
