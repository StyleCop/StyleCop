...
namespace StyleCop.CSharp.Rules
{
    ...
    public class DocumentationRules : SourceAnalyzer
    {
        ...
        /// <summary>
        /// Extracts the generic type argument out of part of a generic type.
        /// </summary>
        /// <param name="fullType">The full generic type.</param>
        /// <param name="startIndex">The start index of the generic type argument.</param>
        /// <param name="endIndex">The index just past the end of the generic type argument. </param>
        ...
        /// </summary>
        /// <param name="xmlHeaderLine">The xml header line token.</param>
        /// <returns>Returns true if the header is empty.</returns>
        private static bool IsXmlHeaderLineEmpty(Comment xmlHeaderLine)
        {
            Param.AssertNotNull(xmlHeaderLine, "xmlHeaderLine");
            Debug.Assert(xmlHeaderLine.CommentType == CommentType.ElementHeaderLine, "The token should be an xml header line");

            int slashCount = 0;
            for (int i = 0; i < xmlHeaderLine.Text.Length; ++i)
            {
                char character = xmlHeaderLine.Text[i];
        ...
        private static string BuildTypeNameStringWithParamsNumber(string typeName)
        {
            ...
            IList<string> genericParams = ExtractGenericParametersFromType(typeName, index);
        ...
        /// <summary>
        /// Builds a regex matching string to match the given generic argument list.
        /// </summary>
        /// <param name="genericParams">The list of generic parameters.</param>
        /// <returns>Returns the regex string.</returns>
        private static string BuildGenericParametersRegex(IList<string> genericParams)
        {
        ...
        private static IList<string> ExtractGenericParametersFromType(string typeName, int index)
        {
            ...
            return genericTypes.AsReadOnly();
        }
        ...
        private void CheckDocumentationForElement(Element element, AnalyzerSettings settings)
        {
            Param.AssertNotNull(element, "element");
            ...
            if (!element.Generated)
            {
                ...
                {
                    this.CheckEnumHeaders(element as StyleCop.CSharp.CodeModel.Enum, settings);
                }
        ...
        private void CheckHeader(Element element, AnalyzerSettings settings, bool partialElement)
        {
            Param.AssertNotNull(element, "element");
            Param.Ignore(settings);
            ...
            ElementHeader header = element.Header;
            if (header == null || header.IsEmpty)
            {
                ...
                if ((element.ElementType == ElementType.Class || element.ElementType == ElementType.Interface || element.ElementType == ElementType.Struct) ||
                    ((settings.RequireFields || element.ElementType != ElementType.Field) &&
                        (!settings.IgnorePrivates || element.AccessModifierType != AccessModifierType.Private) &&
            ...
            {
                this.ParseHeader(element, header, element.LineNumber, partialElement);
            }
        }
        ...
        /// </summary>
        /// <param name="element">The element to check.</param>
        private void CheckElementComments(Element element)
        {
            Param.AssertNotNull(element, "element");

            Token startToken = element.FirstDeclarationToken;
            if (startToken != null)
            {
                for (ElementHeaderLine headerLine = startToken.FindNextDescendentOf<ElementHeaderLine>(element); 
                    headerLine != null; 
                    headerLine = headerLine.FindNextDescendentOf<ElementHeaderLine>(element))
                {
                    this.AddViolation(
                        element,
        ...
        /// </summary>
        /// <param name="element">The element to check.</param>
        private void CheckInheritDocRules(Element element)
        {
            Param.AssertNotNull(element, "element");
        ...
        private void CheckHeaderSummary(Element element, int lineNumber, bool partialElement, XmlDocument formattedDocs)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertGreaterThanZero(lineNumber, "lineNumber");
            Param.Ignore(partialElement);
        ...
        private void CheckDocumentationValidity(Element element, int lineNumber, XmlNode documentationXml, string documentationType)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertGreaterThanZero(lineNumber, "lineNumber");
        ...
        /// <param name="element">The element to check.</param>
        /// <param name="formattedDocs">The formatted header documentation.</param>
        private void CheckPropertyValueTag(Element element, XmlDocument formattedDocs)
        {
            Param.AssertNotNull(element, "element");
        ...
        private void CheckForRepeatingComments(Element element, XmlDocument formattedDocs)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertNotNull(formattedDocs, "formattedDocs");
        ...
        private void CheckHeaderParams(Element element, ParameterList parameters, XmlDocument formattedDocs)
        {
            Param.AssertNotNull(element, "element");
        ...
        /// <param name="formattedDocs">The formatted Xml document that comprises the header.</param>
        private void CheckHeaderReturnValue(Element element, TypeToken returnType, XmlDocument formattedDocs)
        {
            Param.AssertNotNull(element, "element");
        ...
        private void CheckFileHeader(CsDocument document)
        {
            ...
            StringProperty companyNameProperty = this.GetSetting(document, DocumentationRules.CompanyNameProperty) as StringProperty;
            if (companyNameProperty != null)
            {
                companyName = companyNameProperty.Value;
            }

            StringProperty copyrightProperty = this.GetSetting(document, DocumentationRules.CopyrightProperty) as StringProperty;
        ...
        private void CheckFileHeader(CsDocument document, string copyright, string companyName)
        {
            ...
            FileHeader header = document.FileHeader;
            string headerXml = header == null ? null : header.HeaderXml;

            if (string.IsNullOrWhiteSpace(headerXml))
            {
                this.Violation(
                    Rules.FileMustHaveHeader,
                    new ViolationContext(document, 1),
                    (c, o) => { this.AddFileHeader(document, copyright, companyName); });
            }
            else
            {
                ...
                try
                {
                    ...
                    xml.LoadXml(string.Format(CultureInfo.InvariantCulture, "<root>{0}</root>", headerXml));
                    ...
                    {
                        this.Violation(
                            Rules.FileHeaderMustShowCopyright,
                            new ViolationContext(document, document.FileHeader.LineNumber),
                            (c, o) => { this.AddFileHeaderCopyright(document, copyright, xml); });
                    }
                    else
                    {
                        ...
                        {
                            this.Violation(
                                Rules.FileHeaderMustHaveCopyrightText,
                                new ViolationContext(document, document.FileHeader.LineNumber),
                                (c, o) => { this.AddFileHeaderCopyright(document, copyright, xml); });
                        }
                        ...
                        {
                            this.Violation(
                                Rules.FileHeaderCopyrightTextMustMatch,
                                new ViolationContext(document, document.FileHeader.LineNumber, copyright),
                                (c, o) => { this.AddFileHeaderCopyright(document, copyright, xml); });
                        }
                        ...
                        if (attribute == null)
                        {
                            this.AddViolation(document, document.FileHeader.LineNumber, Rules.FileHeaderMustContainFileName);
                        }
                        else
                        {
                            // Make sure the filename matches the name of the file.
                            if (string.Compare(attribute.InnerText, document.Name, StringComparison.OrdinalIgnoreCase) != 0)
                            {
                                this.AddViolation(document, document.FileHeader.LineNumber, Rules.FileHeaderFileNameDocumentationMustMatchFileName);
                            }
                        }
                        ...
                        if (attribute == null)
                        {
                            this.AddViolation(document, document.FileHeader.LineNumber, Rules.FileHeaderMustHaveValidCompanyText);
                        }
                        else
                        {
                            ...
                            {
                                this.AddViolation(document, document.FileHeader.LineNumber, Rules.FileHeaderCompanyNameTextMustMatch, companyName);
                            }
                        }
                    }
                    ...
                    {
                        this.AddViolation(document, document.FileHeader.LineNumber, Rules.FileHeaderMustHaveSummary);
                    }
                }
                catch (XmlException)
                {
                    this.AddViolation(document, 1, Rules.FileMustHaveHeader);
                }
                catch (ArgumentException)
                {
                    this.AddViolation(document, 1, Rules.FileMustHaveHeader);
        ...
        /// <summary>
        /// Adds or fixes the copyright tag within a file header.
        /// </summary>
        /// <param name="document">The document.</param>
        /// <param name="copyright">The required copyright text.</param>
        /// <param name="headerXml">The XML forming the header.</param>
        private void AddFileHeaderCopyright(CsDocument document, string copyright, XmlDocument headerXml)
        {
            Param.AssertNotNull(document, "document");
            Param.Ignore(copyright);
            Param.AssertNotNull(headerXml, "headerXml");
        ...
        private bool InsertIncludedDocumentation(Element element, string basePath, XmlDocument documentation)
        {
            Param.AssertNotNull(element, "element");
        ...
        /// <returns>Returns false if a violation was found.</returns>
        private bool InsertIncludedDocumentationForNode(Element element, string basePath, XmlNode documentationNode)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertValidString(basePath, "basePath");
            Param.AssertNotNull(documentationNode, "documentationNode");
        ...
        private bool InsertIncludedDocumentationForChildNodes(Element element, string basePath, XmlNode documentationNode)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertValidString(basePath, "basePath");
            Param.AssertNotNull(documentationNode, "documentationNode");
        ...
        private bool LoadAndReplaceIncludeTag(Element element, string basePath, XmlNode documentationNode)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertValidString(basePath, "basePath");
            Param.AssertNotNull(documentationNode, "documentationNode");
        ...
        private bool ReplaceIncludeTagWithIncludedDocumentationContents(Element element, XmlNode documentationNode, CachedXmlDocument includedDocument, XmlNode includedDocumentationNode)
        {
            Param.AssertNotNull(element, "element");
            Param.AssertNotNull(documentationNode, "documentationNode");
